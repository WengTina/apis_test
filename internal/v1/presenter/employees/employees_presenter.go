...
func (p *presenter) Created(ctx *gin.Context) {
	...
	codeMessage := p.PurchasesResolver.Created(trx, input)
...
// @param purchases query string false "帳號"
...
func (p *presenter) List(ctx *gin.Context) {
	...
	codeMessage := p.PurchasesResolver.List(input)
	ctx.JSON(http.StatusOK, codeMessage)
}
...
// @produce json
// @param Authorization header string  true "JWE Token"
// @param purchasesID path string true "使用者ID"
// @success 200 object code.SuccessfulMessage{body=purchases.Single} "成功後返回的值"
// @failure 415 object code.ErrorMessage{detailed=string} "必要欄位帶入錯誤"
// @failure 500 object code.ErrorMessage{detailed=string} "伺服器非預期錯誤"
// @Router /v1.0/authority/purchases/{purchasesID} [get]
func (p *presenter) GetByID(ctx *gin.Context) {
	purchasesID := ctx.Param("purchasesID")
	input := &purchases.Field{}
	input.PurchasesID = purchasesID
	if err := ctx.ShouldBindQuery(input); err != nil {
		log.Error(err)
		ctx.JSON(http.StatusOK, code.GetCodeMessage(code.FormatError, err.Error()))
	...
	codeMessage := p.PurchasesResolver.GetByID(input)
	ctx.JSON(http.StatusOK, codeMessage)
}
...
// @failure 415 object code.ErrorMessage{detailed=string} "必要欄位帶入錯誤"
// @failure 500 object code.ErrorMessage{detailed=string} "伺服器非預期錯誤"
// @Router /v1.0/authority/purchases/{purchasesID} [delete]
func (p *presenter) Delete(ctx *gin.Context) {
	...
	codeMessage := p.PurchasesResolver.Deleted(input)
	ctx.JSON(http.StatusOK, codeMessage)
}
...
// @failure 415 object code.ErrorMessage{detailed=string} "必要欄位帶入錯誤"
// @failure 500 object code.ErrorMessage{detailed=string} "伺服器非預期錯誤"
// @Router /v1.0/authority/purchases/{purchasesID} [patch]
func (p *presenter) Updated(ctx *gin.Context) {
	...
	codeMessage := p.PurchasesResolver.Updated(input)
	ctx.JSON(http.StatusOK, codeMessage)
}
